...
import { loginRaPortal, uniqueDid } from "../../lib/fixtures";
...
test.describe("RA Portal — User Management", () => {
  ...
  test("UC-RA-04: filter users by role", async ({ page }) => {
    ...
    const officerDid = uniqueDid("officer-filter");
  ...
  test("UC-RA-05: suspend user removes from list", async ({ page }) => {
    const did = uniqueDid("suspend-me");
    await page.fill("#user-did", did);

defmodule MlDsaAsymkeyTest do
  ...
  test "ML-DSA Key generation and encoding/decoding via protocol" do
    for c <-
          %MlDsaKeypair{}
  ...
  test "ML-DSA sign and verify data using raw keypair" do
    for c <-
          %MlDsaKeypair{}
          |> ContextConfig.get(:supported_variant) do
      ...
      tbs_data = "this is test input for signing for ML-DSA variant #{c}"
      ...
      with ctx when not is_tuple(ctx) <-
        ...
        {:ok, signRes2} =
          %MlDsaContext{private_key: kp.private_key}
          |> AsymkeySign.sign_init()
          |> AsymkeySign.sign_update(tbs_data)
          |> AsymkeySign.sign_final()

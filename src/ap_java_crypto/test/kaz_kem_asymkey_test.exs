defmodule KazKemAsymkeyTest do
  ...
  test "KAZ-KEM Key generation and encoding/decoding via protocol" do
    Enum.each(ContextConfig.get(%KazKemKeypair{}, :supported_variant), fn algo ->
      {:ok, kp} =
        %KazKemKeypair{}
  ...
  test "KAZ-KEM encrypt and decrypt data using raw keypair" do
    for c <-
          %KazKemKeypair{}
          |> ContextConfig.get(:supported_variant) do
      IO.puts("Using KAZ-KEM #{c} for encrypt & decrypt test")

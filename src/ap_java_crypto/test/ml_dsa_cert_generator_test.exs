defmodule MlDsaCertGeneratorTest do
  ...
  setup do
    ...
    {:ok, %{private_key: mldsa_65_priv, public_key: mldsa_65_pub}} =
      %MlDsaKeypair{variant: :ml_dsa_65}
      |> Asymkey.generate()

    {:ok, %{private_key: mldsa_87_priv, public_key: mldsa_87_pub}} =
      %MlDsaKeypair{variant: :ml_dsa_87}
      |> Asymkey.generate()
  ...
  test "ML-DSA generates self-sign issuer X.509 certificate", ctx do
    Enum.each(ContextConfig.get(%MlDsaKeypair{}, :supported_variant), fn algo ->
      co =
        %CertOwner{}
        |> CertOwner.set_name("ML-DSA Root Issuer")
  ...
  test "ML-DSA generates self-sign end user X.509 certificate", ctx do
    Enum.each(ContextConfig.get(%MlDsaKeypair{}, :supported_variant), fn algo ->
  ...
  test "ML-DSA generates self-sign issuer X.509 certificate from CSR", ctx do
    Enum.each(ContextConfig.get(%MlDsaKeypair{}, :supported_variant), fn algo ->
      co =
        %CertOwner{}
        |> CertOwner.set_name("ML-DSA Root CSR Issuer")
        |> CertOwner.set_org("Antrapol")
        |> CertOwner.set_email("root@issuer.com")
  ...
  test "ML-DSA generates self-sign issuer, issue sub CA and subscriber X.509 certificate",
       ctx do
    Enum.each(ContextConfig.get(%MlDsaKeypair{}, :supported_variant), fn algo ->
      Enum.each(ContextConfig.get(%MlDsaKeypair{}, :supported_variant), fn algo2 ->
        pubkey = Map.get(ctx, algo).pubkey
        privkey = Map.get(ctx, algo).privkey

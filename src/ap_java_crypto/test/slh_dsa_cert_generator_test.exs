defmodule SlhDsaCertGeneratorTest do
  ...
  setup do
    ...
    Enum.reduce(ContextConfig.get(%SlhDsaKeypair{}, :supported_variant), res, fn algo, acc ->
      assert {:ok, %{private_key: priv, public_key: pub}} =
               %SlhDsaKeypair{variant: algo}
               |> Asymkey.generate()

      Map.put(acc, algo, %{pubkey: pub, privkey: priv})
    end)

    # {:ok, %{private_key: mldsa_44_priv, public_key: mldsa_44_pub}} =
    #  %SlhDsaKeypair{}
    #  |> Asymkey.generate()

    # {:ok, %{private_key: mldsa_65_priv, public_key: mldsa_65_pub}} =
    #  %SlhDsaKeypair{variant: :ml_dsa_65}
    #  |> Asymkey.generate()

    # {:ok, %{private_key: mldsa_87_priv, public_key: mldsa_87_pub}} =
    #  %SlhDsaKeypair{variant: :ml_dsa_87}
    #  |> Asymkey.generate()
    ...
    #  subca2: %{pubkey: subsubca_pubkey, privkey: subsubca_privkey},
    #  subscriber: %{pubkey: subscriber_pubkey, privkey: subscriber_privkey},
    #  ml_dsa_44: %{pubkey: mldsa_44_pub, privkey: mldsa_44_priv},
    #  ml_dsa_65: %{pubkey: mldsa_65_pub, privkey: mldsa_65_priv},
    #  ml_dsa_87: %{pubkey: mldsa_87_pub, privkey: mldsa_87_priv}
    # }
  end

  test "SLH-DSA generates self-sign issuer X.509 certificate", ctx do
    Enum.each(ContextConfig.get(%SlhDsaKeypair{}, :supported_variant), fn algo ->
      co =
        %CertOwner{}
        |> CertOwner.set_name("SLH-DSA Root Issuer")
  ...
  test "SLH-DSA generates self-sign end user X.509 certificate", ctx do
    Enum.each(ContextConfig.get(%SlhDsaKeypair{}, :supported_variant), fn algo ->
      ...
      File.write!("self-sign-end-user-#{algo}.crt", cert)
      File.write!("self-sign-end-user-#{algo}.pem", KeyEncoding.encode!(privkey, :pem).value)
  ...
  test "SLH-DSA generates self-sign issuer X.509 certificate from CSR", ctx do
    Enum.each(ContextConfig.get(%SlhDsaKeypair{}, :supported_variant), fn algo ->
      co =
        %CertOwner{}
        |> CertOwner.set_name("SLH-DSA Root CSR Issuer")
        |> CertOwner.set_org("Antrapol")
        |> CertOwner.set_email("root@issuer.com")
  ...
  test "SLH-DSA generates self-sign issuer, issue sub CA and subscriber X.509 certificate",
       ctx do
    Enum.each(ContextConfig.get(%SlhDsaKeypair{}, :supported_variant), fn algo ->
      Enum.each(ContextConfig.get(%SlhDsaKeypair{}, :supported_variant), fn algo2 ->
        pubkey = Map.get(ctx, algo).pubkey
        privkey = Map.get(ctx, algo).privkey

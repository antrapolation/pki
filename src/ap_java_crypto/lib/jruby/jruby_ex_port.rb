module JrubyExPort
  ...
  class JrubyExPortException < StandardError; end
  ...
  class PortProcess
    ...
    def register(ops, &block)
      res = @callback[ops.to_sym] = block
      puts "Service end point #{ops} is registered"
      res
    end
    ...
    def start
      ...
      java_import com.ericsson.otp.erlang.OtpErlangBoolean
    ...
    def reply_caller(reply, *value)
      # puts "reply caller with reply / value : #{reply} / #{value}"
      vv = value.collect do |v|
        to_otp_erlang_object(v)
      end
    ...
    def to_otp_erlang_object(v)
      case v
      when Symbol
        OtpErlangAtom.new(v.to_s)

      when Java::byte[]
        OtpErlangBinary.new(v)

      when Array
        jv = v.collect do |vv|
          to_otp_erlang_object(vv)
        end

        OtpErlangList.new(jv.to_java(OtpErlangObject))

      when Hash
        OtpErlangMap.new(v.keys.collect {|kk| to_otp_erlang_object(kk)}.to_java(OtpErlangObject), v.values.collect {|c| to_otp_erlang_object(c)}.to_java(OtpErlangObject))

      when Integer, Float
        OtpErlangDouble.new(v)

      when String 
        OtpErlangBinary.new(v.to_java.get_bytes("UTF-8"))

      when FalseClass
        OtpErlangBoolean.new(false)

      when TrueClass
        OtpErlangBoolean.new(true)

      else
        puts "Unhandled jruby to erlang object : #{v.class}"
        if v.nil?
          OtpErlangString.new('')
        elsif !v.is_a?(OtpErlangObject)

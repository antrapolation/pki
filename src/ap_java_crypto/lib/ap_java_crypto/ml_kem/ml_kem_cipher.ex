defmodule ApJavaCrypto.MlKem.MlKemCipher do
  ...
  def set_cipher(ctx, cipher) do
    %MlKemCipher{ctx | cipher: cipher}
  end

  def clear_cipher(%MlKemCipher{} = cipher) do
    %MlKemCipher{cipher | cipher: nil}
  end

  def get_cipher(ctx), do: ctx[:cipher]

  def set_cipher_context(ctx, cipher_context),
    do: %MlKemCipher{ctx | cipher_context: cipher_context}

  def set_kdf_context(ctx, kdf_context),
    do: %MlKemCipher{ctx | kdf_context: kdf_context}

  def set_recp_envp(ctx, envp) when is_list(envp) do
    %MlKemCipher{ctx | recp_envp: envp}
  end

  def add_recp_envp(ctx, envp) when not is_list(envp) do
    %MlKemCipher{ctx | recp_envp: ctx.recp_envp ++ [envp]}
  end
  ...
  def set_session_info(ctx, key, value) do
    case Map.has_key?(ctx.session_info, key) do
      true ->
        %MlKemCipher{ctx | session_info: %{ctx.session_info | key => value}}

      false ->
        %MlKemCipher{ctx | session_info: Map.put_new(ctx.session_info, key, value)}
    end
  end
...
defimpl ExCcrypto.Asymkey.AsymkeyDecrypt, for: MlKemCipher do
  ...
  def decrypt_init(ctx, %MlKemPublicKey{} = public_key, %MlKemPrivateKey{} = private_key, _opts) do
    kid = UniqueKeyIdGenerator.unique_key_id(public_key)

    case MlKemCipher.is_recipient?(ctx, kid) do
      true ->
        %MlKemEnvp{} = envp = MlKemCipher.get_recipient_envp(ctx, kid)
  ...
  def decrypt_update(ctx, data) do
    MlKemCipher.set_session_info(
      ctx,
      :session_cipher,
      Cipher.cipher_update(MlKemCipher.get_session_info(ctx, :session_cipher), data)
    )
  end

  def decrypt_final(ctx, _opts) do
    with {:ok, plain} <- Cipher.cipher_final(MlKemCipher.get_session_info(ctx, :session_cipher)) do
      plain
    end
  end

defmodule ApJavaCrypto.SlhDsa.SlhDsaContext do
...
defimpl AsymkeySign, for: SlhDsaContext do
  ...
  def sign_final(%SlhDsaContext{} = ctx, nil) do
    with res <- :crypto.hash_final(ctx.digest_ctx),
      ...
      {:ok,
       %SlhDsaEnvp{}
       |> SlhDsaEnvp.set_signature(signature)
       |> SlhDsaEnvp.set_sign_context(ctx)
       |> SlhDsaEnvp.sanitize()}
    end
  end

  def sign_final(%SlhDsaContext{} = ctx, data), do: sign_update(ctx, data)
...
defimpl ExCcrypto.ContextConfig, for: SlhDsaContext do
  ...
  def set(ctx, :private_key, value, _opts), do: %SlhDsaContext{ctx | private_key: value}
  def set(ctx, :digest_algo, value, _opts), do: %SlhDsaContext{ctx | digest_algo: value}
  ...
  def info(_ctx, info),
    do: %{error: "Info operation error on SlhDsaContext. No info key '#{info}' found"}
end

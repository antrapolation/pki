defmodule ApJavaCrypto.MlDsa.MlDsaVerifyContext do
  alias ApJavaCrypto.MlDsa.MlDsaVerifyContext
  ...
  def reset_digest_context(%MlDsaVerifyContext{} = ctx), do: %{ctx | digest_context: nil}
end

alias ApJavaCrypto.MlDsa.MlDsaVerifyContext

defimpl ExCcrypto.ContextConfig, for: MlDsaVerifyContext do
  def set(ctx, :verification_key, value, _opts),
    do: MlDsaVerifyContext.set_verification_key(ctx, value)

  def set(ctx, :digest_algo, value, _opts),
    do: MlDsaVerifyContext.set_digest_algo(ctx, value)
  ...
  def info(_ctx, info),
    do: %{error: "Info operation error on MlDsaVerifyContext. No info key '#{info}' found"}
end

# alias ExCcrypto.Asymkey.AsymkeyVerify

# defimpl AsymkeyVerify, for: MlDsaVerifyContext do
#  require X509.ASN1
#  alias ExCcrypto.Asymkey.MlDsa.MlDsaPublicKey
#  alias ExCcrypto.Asymkey.MlDsa.MlDsaVerifyContext
#  alias ExCcrypto.Asymkey.AsymkeyHelper
#
#  def verify_init(
#        %MlDsaVerifyContext{verification_key: X509.ASN1.otp_certificate()} = ctx,
#        opts
#      ) do
#    verify_init(
...
#  end
#
#  def verify_init(%MlDsaVerifyContext{verification_key: %MlDsaPublicKey{}} = ctx, _opts) do
...
#    # )
#
#    {:ok, %{verification_result: verRes, context: MlDsaVerifyContext.reset_digest_context(ctx)}}
#  end
#

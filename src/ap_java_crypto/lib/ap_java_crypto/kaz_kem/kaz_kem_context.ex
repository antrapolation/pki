defmodule ApJavaCrypto.KazKem.KazKemContext do
  ...
  def add_encryption_key(ctx, %KazKemPublicKey{} = key, opts) do
    %{
      ctx
      | encryption_keys:
          ctx.encryption_keys ++
...
alias ApJavaCrypto.KazKem.KazKemContext

defimpl ExCcrypto.ContextConfig, for: KazKemContext do
  def set(ctx, :add_encryption_key, val, _opts),
    do: KazKemContext.add_encryption_key(ctx, val)
  ...
  def info(_ctx, :getter_key) do
    %{
      cipher_context: "Return the cipher context for this KAZ-KEM operation",
      kdf_context: "Return the KDF config to derive the session key for KAZ-KEM operation"
    }
  end

  def info(_ctx, :setter_key),
    do: %{
      add_encryption_key: "Add recipient key to construct the KAZ-KEM envelope"
    }
...
defimpl ExCcrypto.Asymkey.AsymkeyEncrypt, for: KazKemContext do
  ...
  def encrypt_init(%KazKemContext{} = ctx, opts) do
  ...
  def encrypt_final(ctx, opts) do
    ...
    {:ok,
     %KazKemCipher{}
     |> KazKemCipher.set_cipher(cipher)
     |> KazKemCipher.set_recp_envp(recp_envp)
     |> KazKemCipher.set_cipher_context(cipher_ctx)
     |> KazKemCipher.set_kdf_context(ctx.kdf_context)}
  end
  ...
  defp encrypt_for_recipient(recp, key) do
    ...
    %KazKemEnvp{}
    |> KazKemEnvp.set_cipher_context(recpCipherCtx)
    |> KazKemEnvp.set_cipher(cipher)
    |> KazKemEnvp.set_kdf_context(ContextConfig.get(kdf_output, :derivation_context))
    |> KazKemEnvp.set_recp_key_id(UniqueKeyIdGenerator.unique_key_id(recpKey))
    |> KazKemEnvp.set_recipient_cipher(recp_cipher)

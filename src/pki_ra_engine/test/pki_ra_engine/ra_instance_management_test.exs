defmodule PkiRaEngine.RaInstanceManagementTest do
  ...
  describe "create_ra_instance/1" do
    test "creates an RA instance with valid attrs" do
      assert {:ok, ra} = RaInstanceManagement.create_ra_instance(@valid_attrs)
    ...
    test "rejects duplicate name" do
      assert {:ok, _} = RaInstanceManagement.create_ra_instance(@valid_attrs)
      assert {:error, changeset} = RaInstanceManagement.create_ra_instance(@valid_attrs)
      assert errors_on(changeset)[:name]
    end

    test "rejects missing name" do
      assert {:error, changeset} = RaInstanceManagement.create_ra_instance(%{created_by: "admin"})
      assert errors_on(changeset)[:name]
    end
  end
  ...
  describe "list_ra_instances/0" do
    ...
    test "returns all created instances" do
      {:ok, _} = RaInstanceManagement.create_ra_instance(%{name: "RA-1", created_by: "admin"})
      {:ok, _} = RaInstanceManagement.create_ra_instance(%{name: "RA-2", created_by: "admin"})
  ...
  describe "get_ra_instance/1" do
    test "returns instance by id" do
      {:ok, ra} = RaInstanceManagement.create_ra_instance(@valid_attrs)
    ...
    test "returns error for non-existent id" do
      assert {:error, :not_found} = RaInstanceManagement.get_ra_instance(Uniq.UUID.uuid7())
    end
  end
  ...
  describe "update_status/2" do
    ...
    test "updates status to suspended" do
      {:ok, ra} = RaInstanceManagement.create_ra_instance(@valid_attrs)
      assert {:ok, updated} = RaInstanceManagement.update_status(ra.id, "suspended")
      assert updated.status == "suspended"
    end

    test "rejects invalid status" do
      {:ok, ra} = RaInstanceManagement.create_ra_instance(@valid_attrs)
      assert {:error, changeset} = RaInstanceManagement.update_status(ra.id, "deleted")
      assert errors_on(changeset)[:status]
    end

    test "returns error for non-existent id" do
      assert {:error, :not_found} = RaInstanceManagement.update_status(Uniq.UUID.uuid7(), "active")
    end
  end
end

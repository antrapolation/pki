# tag 0x12
defmodule StrapCiphagile.Context.PrivateKey do
  use TypedStruct
  ...
  typedstruct do
    ...
    field(:variant, any())
    field(:format, any())
    # TLV SymkeyCipher or AsymkeyCipher struct
    field(:enc_key_value, any())
    # TLV 0xF0
    field(:misc, any())
  end
...
defimpl EncoderProtocol, for: PrivateKey do
  def encode(%PrivateKey{} = ctx, _opts) do
    with {:ok, version_byte} <- Encodings.encode_version(ctx.version),
         ...
         {:ok, priv_key_bin} <- Encodings.encode_tlv(0x01, ctx.enc_key_value),
         {:ok, misc_bin} <- Encodings.encode_tlv(0xF0, ctx.misc) do
      content = <<version_byte, algo_byte, variant_byte, format_byte>> <> priv_key_bin <> misc_bin

      with {:ok, full_data} <- VarLengthData.encode(content),
           {:ok, tag} <- Tags.tag_value(:privkey_envp) do
        {:ok, <<tag>> <> full_data}
      else
        err -> err
      end
...
defimpl DecoderProtocol, for: PrivateKey do
  def decode(%PrivateKey{} = _ctx, bin, _opts) do
    with true <- match?(<<0x12, _::binary>>, bin),
         ...
         {:ok, privkey_struct} <-
           decode_tlv(inner_rest, %PrivateKey{
             version: version,
             algo: algo,
             variant: variant,
             format: format
  ...
  defp decode_tlv(<<0x01, rest::binary>>, acc) do
    with {:ok, val, rest_after} <- VarLengthData.decode(rest) do
      decode_tlv(rest_after, %{acc | enc_key_value: val})
    end
  end

defmodule PkiRaPortalWeb.Plugs.RateLimiterTest do
  use PkiRaPortalWeb.ConnCase

  alias PkiRaPortalWeb.Plugs.RateLimiter

  describe "allows requests under the limit" do
    test "first requests pass through", %{conn: conn} do
      prefix = "test_ra_portal_#{System.unique_integer([:positive])}"
  ...
  describe "denies requests over the limit" do
    test "returns 429 when limit exceeded", %{conn: conn} do
      prefix = "test_ra_portal_deny_#{System.unique_integer([:positive])}"
    ...
    test "429 response includes retry-after header", %{conn: conn} do
      prefix = "test_ra_portal_retry_#{System.unique_integer([:positive])}"
    ...
    test "429 response renders login page with error message", %{conn: conn} do
      prefix = "test_ra_portal_msg_#{System.unique_integer([:positive])}"
  ...
  describe "client_ip proxy handling" do
    test "uses X-Forwarded-For when behind trusted proxy" do
      prev = Application.get_env(:pki_ra_portal, :trusted_proxies)
      Application.put_env(:pki_ra_portal, :trusted_proxies, ["127.0.0.1"])

      prefix = "test_ra_portal_xff_#{System.unique_integer([:positive])}"
      opts = RateLimiter.init(key_prefix: prefix, limit: 1, scale_ms: 60_000)
      ...
      if prev, do: Application.put_env(:pki_ra_portal, :trusted_proxies, prev),
        else: Application.delete_env(:pki_ra_portal, :trusted_proxies)
    end

    test "multi-proxy X-Forwarded-For skips trusted proxies right-to-left" do
      prev = Application.get_env(:pki_ra_portal, :trusted_proxies)
      Application.put_env(:pki_ra_portal, :trusted_proxies, ["127.0.0.1", "10.0.0.1"])

      prefix = "test_ra_portal_multi_#{System.unique_integer([:positive])}"
      opts = RateLimiter.init(key_prefix: prefix, limit: 1, scale_ms: 60_000)
  ...
  describe "different IPs have independent limits" do
    test "separate IPs are tracked independently" do
      prefix = "test_ra_portal_indep_#{System.unique_integer([:positive])}"
      opts = RateLimiter.init(key_prefix: prefix, limit: 1, scale_ms: 60_000)
  ...
  describe "rate_limit_enabled flag" do
    test "bypasses rate limiting when disabled", %{conn: conn} do
      prev = Application.get_env(:pki_ra_portal, :rate_limit_enabled)
      Application.put_env(:pki_ra_portal, :rate_limit_enabled, false)

      prefix = "test_ra_portal_disabled_#{System.unique_integer([:positive])}"
      opts = RateLimiter.init(key_prefix: prefix, limit: 1, scale_ms: 60_000)
      ...
      if prev, do: Application.put_env(:pki_ra_portal, :rate_limit_enabled, prev),
        else: Application.delete_env(:pki_ra_portal, :rate_limit_enabled)
    end
  end
end

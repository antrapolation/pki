defmodule PkiRaPortalWeb.Plugs.RateLimiter do
  @moduledoc """
  ...
  trusted proxy. Configure via :pki_ra_portal, :trusted_proxies (list of IP strings).
  """
  ...
  defp rate_limit_enabled? do
    Application.get_env(:pki_ra_portal, :rate_limit_enabled, true)
  end
  ...
  defp do_rate_limit(conn, opts) do
    ...
    case Hammer.check_rate(key, scale_ms, limit) do
      ...
      {:deny, _limit} ->
        ...
        |> put_status(429)
        |> Phoenix.Controller.put_view(PkiRaPortalWeb.SessionHTML)
  ...
  defp client_ip(conn) do
    ...
    trusted = Application.get_env(:pki_ra_portal, :trusted_proxies, [])

defmodule PkiRaPortalWeb.Plugs.RequireAuth do
  @moduledoc """
  ...
  def call(conn, _opts) do
    ...
    with {:ok, session_id} <- ensure_present(session_id),
         {:ok, session} <- PkiRaPortal.SessionStore.lookup(session_id),
      ...
      {:error, :expired} ->
        ...
        PkiPlatformEngine.PlatformAudit.log("session_expired", %{
          portal: "ra", details: %{session_id: session_id}
        })
  ...
  defp check_user_agent(conn, session_id, session) do
    ...
    if current_ua == session.user_agent do
      ...
      PkiRaPortal.SessionSecurity.notify(:session_hijack_suspected, %{
  ...
  defp check_ip(conn, _session_id, session) do
    if not Application.get_env(@app, :session_ip_pinning, true) do
      ...
      if current_ip == session.ip do
        ...
        PkiRaPortal.SessionStore.update_ip(session.session_id, current_ip)

        PkiRaPortal.SessionSecurity.notify(:session_ip_changed, %{
          username: session.username,
          role: session.role,
          old_ip: session.ip,
          new_ip: current_ip,

defmodule StrapJavaCryptoPrivKeyStoreProvider.JavaCryptoProviderProcess do
  ...
  # Delegate encrypt/decrypt to encapsulation/decapsulation for KEM support
  def handle_call({:encrypt_data, ctx, data, opts}, _from, state) do
    {:reply, PublicKeyOps.encrypt_data(ctx, data, opts), state}
  end

  def handle_call({:decrypt_data, ctx, pubkey, cipher, opts}, _from, state) do
    {:reply, PrivateKeyOps.decrypt_data(ctx, pubkey, cipher, opts), state}
  end
end

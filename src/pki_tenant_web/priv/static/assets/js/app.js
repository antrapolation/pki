...
`
  ...
  var SessionTimeout = {
    mounted() {
      this.warningMs = parseInt(this.el.dataset.warningMs) || 25 * 60 * 1e3;
      this.timeoutMs = parseInt(this.el.dataset.timeoutMs) || 30 * 60 * 1e3;
    ...
    submitLogout() {
      ...
      form.method = "post";
      form.action = "/logout";
      form.innerHTML = `<input type="hidden" name="_method" value="delete"><input type="hidden" name="_csrf_token" value="${csrfToken2}">`;
      document.body.appendChild(form);
      form.submit();
    },
    showWarning() {
      ...
      if (modal) {
        ...
        this.countdownInterval = setInterval(() => {
          ...
          if (left <= 0) {
            ...
            const secs = Math.floor(left % 6e4 / 1e3);
            if (countdown) {
  ...
  var session_timeout_default = SessionTimeout;
  ...
  var download_hook_default = DownloadHook;
  ...
  var liveSocket = new LiveSocket2("/live", Socket, {
    ...
    hooks: { SessionTimeout: session_timeout_default, DownloadHook: download_hook_default }
  });

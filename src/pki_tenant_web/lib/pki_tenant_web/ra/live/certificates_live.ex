defmodule PkiTenantWeb.Ra.CertificatesLive do
  use PkiTenantWeb, :live_view

  alias PkiMnesia.{Repo, Structs.IssuedCertificate}
  ...
  def handle_event("view_cert", %{"serial" => serial}, socket) do
    case Repo.where(IssuedCertificate, fn c ->
      (c.serial_number == serial) or (c.issued_cert_serial == serial)
    end) do
      {:ok, [cert | _]} ->
  ...
  def handle_event("revoke_cert", _, socket) do
    ...
    if role != "ra_admin" do
      ...
      # Update the certificate status in Mnesia directly
      case Repo.update(cert, %{status: "revoked", revocation_reason: reason, revoked_at: DateTime.utc_now() |> DateTime.truncate(:second)}) do
        {:ok, _result} ->
  ...
  defp load_certificates(socket) do
    ...
    certs = case Repo.all(IssuedCertificate) do
      {:ok, all_certs} ->
        if status != "all" do
          Enum.filter(all_certs, fn c -> c.status == status end)
        else
          all_certs
        end
      _ -> []

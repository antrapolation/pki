defmodule PkiTenantWeb.Ra.SetupWizardLive do
  use PkiTenantWeb, :live_view

  require Logger

  alias PkiRaEngine.{CertProfileConfig, ApiKeyManagement}
  ...
  def handle_event("connect_key", params, socket) do
    ...
    conn = RaCaConnection.new(attrs)

    case Repo.insert(conn) do
      {:ok, _conn} ->
        send(self(), :load_data)
  ...
  def handle_event("create_api_key", params, socket) do
    ...
    case ApiKeyManagement.create_api_key(attrs) do
      {:ok, key, raw_key} ->
        ...
        {:noreply,
         socket
         |> assign(created_api_keys: keys, raw_key_display: raw_key)

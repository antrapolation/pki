defmodule PkiTenantWeb.Ra.DashboardLive do
  ...
  alias PkiMnesia.{Repo, Structs.RaCaConnection, Structs.RaInstance}
  ...
  defp load_admin_data(socket) do
    ...
    # CSRs
    csrs =
      case CsrValidation.list_csrs() do
        {:ok, c} -> c
        _ -> []
      end

    pending_csrs = Enum.filter(csrs, &(&1.status == "verified"))
    stuck_csrs = Enum.filter(csrs, &(&1.status == "approved"))

    # API keys
    api_keys =
      case PkiRaEngine.ApiKeyManagement.list_api_keys() do
        {:ok, ak} -> ak
        _ -> []
      end
  ...
  defp load_officer_data(socket) do
    csrs =
      case CsrValidation.list_csrs() do
        {:ok, c} -> c
        _ -> []
    ...
    verified_csrs = Enum.filter(csrs, &(&1.status == "verified"))
  ...
  defp load_auditor_data(socket) do
    csrs =
      case CsrValidation.list_csrs() do
        {:ok, c} -> c
        _ -> []
    ...
    pending_count = csrs |> Enum.filter(&(&1.status == "verified")) |> length()
    approved_count = csrs |> Enum.filter(&(&1.status == "approved")) |> length()
              ...
              <tr :for={csr <- @paged_items} class="hover:bg-base-200/50 border-base-300">
                ...
                <td>
                  <span class={[
                    ...
                    csr.status == "verified" && "badge-info",

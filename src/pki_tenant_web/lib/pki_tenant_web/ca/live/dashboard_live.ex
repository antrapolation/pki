defmodule PkiTenantWeb.Ca.DashboardLive do
  ...
  alias PkiMnesia.Structs.{PortalUser, KeyCeremony}
  ...
  def handle_info(:load_data, socket) do
    ...
    {status, keys, ceremonies, keystores, users} =
      if ca_id do
        status = safe_call(fn -> get_engine_status(ca_id) end,
    ...
    non_admin_users = Enum.filter(users, fn u -> u.role != :ca_admin and u.role != "ca_admin" end)
  ...
  # Simple engine status — no GenServer for this yet, build from data
  defp get_engine_status(ca_id) do
    keys = case IssuerKeyManagement.list_issuer_keys(ca_id) do
      {:ok, k} -> k
      _ -> []
    end
    active_count = Enum.count(keys, fn k -> k.status == "active" end)
    {:ok, %{status: "running", uptime_seconds: 0, active_keys: active_count}}
  end

  defp list_ceremonies(ca_id) do
    case Repo.get_all_by_index(KeyCeremony, :ca_instance_id, ca_id) do
      {:ok, ceremonies} -> {:ok, ceremonies}
      {:error, _} -> {:ok, []}
    end
  end

  defp list_portal_users do
            ...
            <table class="table table-sm table-fixed w-full">
              ...
              <tbody>
                <tr :for={ceremony <- paginated_ceremonies} class="hover">
                  ...
                  <td>{ceremony.ceremony_type || "key_ceremony"}</td>
                  <td>

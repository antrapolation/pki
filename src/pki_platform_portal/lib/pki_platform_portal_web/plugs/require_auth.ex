defmodule PkiPlatformPortalWeb.Plugs.RequireAuth do
  ...
  def call(conn, _opts) do
    ...
    with {:ok, session_id} <- ensure_present(session_id),
         {:ok, session} <- PkiPlatformPortal.SessionStore.lookup(session_id),
      ...
      {:error, :expired} ->
        ...
        PkiPlatformEngine.PlatformAudit.log("session_expired", %{
          portal: "platform", details: %{session_id: session_id}
        })
  ...
  defp check_user_agent(conn, session_id, session) do
    ...
    if current_ua == session.user_agent do
      ...
      PkiPlatformPortal.SessionSecurity.notify(:session_hijack_suspected, %{
        username: session.username,
        role: session.role,
  ...
  defp check_ip(conn, _session_id, session) do
    if not Application.get_env(@app, :session_ip_pinning, true) do
      ...
      if current_ip == session.ip do
        ...
        PkiPlatformPortal.SessionStore.update_ip(session.session_id, current_ip)

        PkiPlatformPortal.SessionSecurity.notify(:session_ip_changed, %{
          username: session.username,
          role: session.role,
          old_ip: session.ip,
          new_ip: current_ip,

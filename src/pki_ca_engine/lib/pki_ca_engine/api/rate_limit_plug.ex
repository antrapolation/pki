defmodule PkiCaEngine.Api.RateLimitPlug do
  ...
  ## Options
    ...
    * `:key_prefix` — bucket prefix (default: `"ca_auth"`)
  ...
  def init(opts) do
    %{
      key_prefix: Keyword.get(opts, :key_prefix, "ca_auth"),
  ...
  defp rate_limit_enabled? do
    Application.get_env(:pki_ca_engine, :rate_limit_enabled, true)
  end

  defp client_ip(conn) do
    remote = conn.remote_ip |> :inet.ntoa() |> to_string()
    trusted = Application.get_env(:pki_ca_engine, :trusted_proxies, [])

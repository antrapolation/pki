# Public Struct
defmodule ExCcrypto.Asymkey.Ecc.EciesCipher do
  ...
  def set_cipher(ctx, cipher) do
    %EciesCipher{ctx | cipher: cipher}
  end

  def clear_cipher(%EciesCipher{} = cipher) do
    %EciesCipher{cipher | cipher: nil}
  end

  def get_cipher(ctx), do: ctx[:cipher]

  def set_cipher_context(ctx, cipher_context),
    do: %EciesCipher{ctx | cipher_context: cipher_context}

  def set_kdf_context(ctx, kdf_context),
    do: %EciesCipher{ctx | kdf_context: kdf_context}

  def set_recp_envp(ctx, envp) when is_list(envp) do
    %EciesCipher{ctx | recp_envp: envp}
  end

  def add_recp_envp(ctx, envp) when not is_list(envp) do
    %EciesCipher{ctx | recp_envp: ctx.recp_envp ++ [envp]}
  end
  ...
  def set_session_info(ctx, key, value) do
    case Map.has_key?(ctx.session_info, key) do
      true ->
        %EciesCipher{ctx | session_info: %{ctx.session_info | key => value}}

      false ->
        %EciesCipher{ctx | session_info: Map.put_new(ctx.session_info, key, value)}
    end
  end
...
defimpl ExCcrypto.Asymkey.AsymkeyDecrypt, for: ExCcrypto.Asymkey.Ecc.EciesCipher do
  ...
  def decrypt_init(ctx, pubkey, %EccPrivateKey{} = privkey, opts) do
  ...
  def decrypt_init(ctx, public_key, private_key, _opts) do
    ...
    case EciesCipher.is_recipient?(ctx, kid) do
      true ->
        envp = EciesCipher.get_recipient_envp(ctx, kid)
        recp_cc = envp.cipher_context

        {{:ECPoint, _} = senderPubKey, {:namedCurve, _} = _curve} =
          KeyEncoding.to_native!(envp.sender_public)

        shared_secret = :public_key.compute_key(senderPubKey, private_key)
  ...
  def decrypt_update(ctx, data) do
    EciesCipher.set_session_info(
      ctx,
      :session_cipher,
      Cipher.cipher_update(EciesCipher.get_session_info(ctx, :session_cipher), data)
    )
  end

  def decrypt_final(ctx, _opts) do
    with {:ok, plain} <- Cipher.cipher_final(EciesCipher.get_session_info(ctx, :session_cipher)) do
      plain
    end
  end

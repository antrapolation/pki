defmodule EccCertGeneratorTest do
  ...
  test "ECC generates self-sign issuer X.509 certificate with multiple hash algo", %{
  ...
  test "ECC generates self-sign issuer X.509 certificate", %{
    root: %{pubkey: pubkey, privkey: privkey}
  } do
    co =
      %CertOwner{}
  ...
  test "ECC generates self-sign end user X.509 certificate", %{
    root: %{pubkey: pubkey, privkey: privkey}
  } do
    co =
      %CertOwner{}
  ...
  test "ECC generates self-sign issuer X.509 certificate from CSR", %{
    root: %{pubkey: pubkey, privkey: privkey}
  } do
    co =
      %CertOwner{}
  ...
  test "ECC generates self-sign issuer and issue sub CA X.509 certificate", %{
    root: %{pubkey: pubkey, privkey: privkey},
    subca: %{pubkey: subca_pubkey, privkey: subca_privkey}
  } do
    root_co =
  ...
  test "ECC generates self-sign issuer, issue sub CA and subscriber X.509 certificate", %{
    ...
    root_co =
      %CertOwner{}
      |> CertOwner.set_name("ECC Root Issuer")
      |> CertOwner.set_org("Antrapol")
      |> CertOwner.set_email("root@issuer.com")
    ...
    euser_co =
      %CertOwner{}
      |> CertOwner.set_name("ECC End User")
      |> CertOwner.set_org("Antrapol")
      |> CertOwner.set_email("subca2@issuer.com")
      |> CertOwner.set_country("MY")
      |> CertOwner.add_org_unit("X Division")
    ...
    File.write!("test_artifacts/subscriber-ecc.p12", p12)
    IO.puts("PKCS12 file written to subscriber-ecc.p12")
  end

  test "ECC generates self-sign issuer, issue sub CA and subscriber X.509 certificate all as external signer",
       %{
         root: %{pubkey: pubkey, privkey: privkey},
         subca: %{pubkey: subca_pubkey, privkey: subca_privkey},
         subscriber: %{

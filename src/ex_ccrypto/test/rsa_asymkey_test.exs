defmodule RsaAsymkeyTest do
  ...
  test "RSA Key generation and encoding/decoding via protocol" do
    for c <-
          AsymkeyContextBuilder.generator_context(:rsa)
          |> ContextConfig.get(:supported_keysizes) do
      {:ok, kp} =
        AsymkeyContextBuilder.generator_context(:rsa)
        |> ContextConfig.set(:keysize, c)
        |> Asymkey.generate()
      ...
      assert {:ok, enc_ks} = Asymkeystore.to_keystore(kp, %{password: "p@ssw0rd"})
      IO.inspect(enc_ks)

      assert {:ok, rks} = AsymkeystoreLoader.from_keystore(enc_ks, %{password: "p@ssw0rd"})
      IO.inspect(rks)
      assert rks == kp
  ...
  test "RSA sign and verify data using raw keypair" do
    for c <-
          AsymkeyContextBuilder.generator_context(:rsa)
          |> ContextConfig.get(:supported_keysizes) do
      IO.puts("Sign verify using keysize : #{c}")
      ...
      tbs_data = "this is test input for signing for RSA keysize #{c}"
  ...
  test "RSA attached sign and verify data using raw keypair" do
    for c <-
          AsymkeyContextBuilder.generator_context(:rsa)
          |> ContextConfig.get(:supported_keysizes) do
      IO.puts("Sign verify using keysize : #{c}")
      ...
      tbs_data = "this is test input for attaached signing using RSA keysize #{c}"
  ...
  test "RSA encrypt and decrypt data using raw keypair" do
    for c <-
          AsymkeyContextBuilder.generator_context(:rsa)
          |> ContextConfig.get(:supported_keysizes) do
      IO.puts("Using keysize #{c} for encrypt & decrypt test")
      ...
      with ctx when not is_tuple(ctx) <-
             AsymkeyEncryptContextBuilder.encrypt_context(:rsa)
             |> ContextConfig.set(:add_encryption_key, kp1.public_key) do
        sensitive_data = "this is sensitive data for RSA enc dec operation"
        ...
        assert(
          case erres do
            {:error, :recipient_decryption_failed} ->
              true

            {:error, {reason, len1, len2}} ->
              reason == :wrong_session_key_length
        ...
        # {:error, {reason, len1, len2}} = erres
        # assert(reason == :wrong_session_key_length)
        # assert(erres == {:error, :recipient_decryption_failed})
        # assert(erres == {:error, {:wrong_session_key_length, _len, _len}})

defmodule StrapSoftPrivateKeystore.SoftMlKemKeypair do
end

alias ExCcrypto.Asymkey.MlKem.MlKemKeypair

defimpl StrapPrivateKeystore.KeypairManager, for: MlKemKeypair do
  ...
  def to_keystore(%MlKemKeypair{} = kp, auth_token, opts) do
  ...
  def public_key(%MlKemKeypair{} = kp, _opts), do: kp.public_key

  def private_key(%MlKemKeypair{} = kp, _opts), do: kp.private_key

  def sign_data(%MlKemKeypair{}, _, _),
    do: {:error, :ml_kem_not_supporting_signing_operation}

  def verify_data(%MlKemKeypair{}, _, _, _),
    do: {:error, :ml_kem_not_supporting_digital_signature_verifying_operation}

  def encrypt_data(%MlKemKeypair{} = kp, data, %{data_feeder: feeder}) when not is_nil(feeder) do
    esess =
  ...
  def encrypt_data(%MlKemKeypair{} = kp, data, _opts) do
    AsymkeyEncryptContextBuilder.encrypt_context(kp.public_key)
    |> AsymkeyEncrypt.encrypt_init()
    |> AsymkeyEncrypt.encrypt_update(data)
  ...
  def decrypt_data(%MlKemKeypair{} = kp, cipher, %{data_feeder: feeder})
  ...
  def decrypt_data(%MlKemKeypair{} = kp, cipher, _opts) do
    with {:ok, ctx} <-
           AsymkeyDecrypt.decrypt_init(cipher, kp.public_key, kp.private_key) do

defmodule StrapSoftPrivateKeystore.SoftKazSignKeypair do
end

alias ExCcrypto.Asymkey.KazSign.KazSignKeypair

defimpl StrapPrivateKeystore.KeypairManager, for: KazSignKeypair do
  alias ExCcrypto.Asymkey.KazSign.KazSignKeypair
  ...
  def to_keystore(%KazSignKeypair{} = kp, auth_token, opts) do
  ...
  def public_key(%KazSignKeypair{} = kp, _opts), do: kp.public_key

  # def private_key(%KazSignKeypair{} = kp, :exccrypto_external_signer) do
  #  %ExternalSigner{}
  ...
  def private_key(%KazSignKeypair{} = kp, _opts), do: kp.private_key

  def sign_data(%KazSignKeypair{} = kp, data, %{data_feeder: feeder})
      when not is_nil(feeder) do
    ssess =
      AsymkeySignContextBuilder.sign_context(:kaz_sign)
  ...
  def sign_data(%KazSignKeypair{} = kp, data, _opts) do
    AsymkeySignContextBuilder.sign_context(:kaz_sign)
    |> ContextConfig.set(:private_key, kp.private_key)
    |> AsymkeySign.sign_init()
    |> AsymkeySign.sign_update(data)
  ...
  def verify_data(%KazSignKeypair{} = kp, data, signature, %{data_feeder: feeder})
      when not is_nil(feeder) do
    vsess =
  ...
  def verify_data(%KazSignKeypair{} = kp, data, signature, _opts) do
    AsymkeyVerify.verify_init(signature, %{
      verification_key: kp.public_key
    })
    |> AsymkeyVerify.verify_update(data)
  ...
  def encrypt_data(%KazSignKeypair{}, _data, _opts),
    do: {:error, :kaz_sign_not_supporting_encrypt_operation}

  def decrypt_data(%KazSignKeypair{}, _cipher, _opts),
    do: {:error, :kaz_sign_not_supporting_decrypt_operation}

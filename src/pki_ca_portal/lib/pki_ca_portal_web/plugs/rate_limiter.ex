defmodule PkiCaPortalWeb.Plugs.RateLimiter do
  @moduledoc """
  ...
  trusted proxy. Configure via :pki_ca_portal, :trusted_proxies (list of IP strings).
  """
  ...
  defp rate_limit_enabled? do
    Application.get_env(:pki_ca_portal, :rate_limit_enabled, true)
  end
  ...
  defp do_rate_limit(conn, opts) do
    ...
    case Hammer.check_rate(key, scale_ms, limit) do
      ...
      {:deny, _limit} ->
        ...
        |> put_status(429)
        |> Phoenix.Controller.put_view(PkiCaPortalWeb.SessionHTML)
      ...
      {:error, reason} ->
        # Fail-closed: if the rate limiter is down, deny the request
  ...
  defp client_ip(conn) do
    ...
    trusted = Application.get_env(:pki_ca_portal, :trusted_proxies, [])

    if remote in trusted do
      case get_req_header(conn, "x-forwarded-for") do
        [forwarded | _] ->
          # Walk right-to-left, skip trusted proxies, take first untrusted IP
          forwarded
          |> String.split(",")
